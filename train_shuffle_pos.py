...
if __name__ == "__main__":
    ...
    # wikipedia-pubmed-and-PMC-w2v.bin PubMed-shuffle-win-30.bin
    parser.add_argument('--emb_file', default='data/PubMed-w2v.bin',
    ...
    parser.add_argument('--pos', action='store_true', help='have pos or not')
    ...
    test_dataset_loader = []
    f_map = dict()
    l_map = [dict() for i in range(file_num)]
    char_count = dict()
    train_features = []
    test_features = []
    train_labels = []
    ...
    for i in range(file_num):  # the number of pos dataset is file_num-1
    ...
    crit_ner = [CRFLoss_vb(l_map_len[i], l_map[i]['<start>'], l_map[i]['<pad>']) for i in range(file_num)]
    ...
    if args.gpu >= 0 and if_cuda is True:
        ...
        if_cuda = False
        packer = [CRFRepack_WC(l_map_len[i], False) for i in range(file_num)]
    ...
    for i in range(file_num-1):
        ...
        predictor.append(predict_wc(if_cuda, f_map, char_map, l_map[i], f_map['<eof>'], char_map['\n'],   # NEW
                                    l_map[i]['<pad>'], l_map[i]['<start>'], True, args.batch_size, args.caseless))
    evaluator.append(eval_wc(packer[file_num-1], l_map[file_num-1], 'a'))   # pos
    ...
    for epoch_idx, args.start_epoch in enumerate(epoch_list):
        ...
        for i in tqdm(range(len(all_indices)), mininterval=2,
            ...
            f_f, f_p, b_f, b_p, w_f, tg_v, mask_v, len_v = next(all_train_data_iter[file_no])  # cycle iteration
        ...
        for file_no in range(file_num-1):

            test_f1, test_pre, test_rec, test_acc = evaluator[file_no].calc_score(ner_model,
                                                                                  test_dataset_loader[file_no], file_no)
            if test_f1 > best_f1_test[file_no]:
                ...
                if args.output_annotation:  # NEW
                    print('annotating')
                    with open('output_3th_sep_pos_coh_' + str(file_num) + '_' + str(file_no) + '.txt', 'w') as fout:
        ...
        # pos
        test_acc = evaluator[file_num-1].calc_score(ner_model, test_dataset_loader[file_num-1],
                                                                     file_num-1)
        ...
        if test_acc > best_acc:
            ...
            track_list.append(
                {'loss': epoch_loss, 'test_acc': test_acc})
            ...
            print('(loss: %.4f, epoch: %d)' %
                  (epoch_loss,
                   args.start_epoch))
            track_list.append({'loss': epoch_loss, 'dev_acc': test_acc})
